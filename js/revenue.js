...
async function exportRevenueRanking(){ const customers=customersForRevenueRanking(); if(!customers.length){toast("Für diese Auswahl sind keine Umsätze vorhanden.");return;} const owner=$("#revenueExportOwner").value; downloadCsvFile(`umsatz-rangliste-${String(owner||"alle").toLowerCase().replace(/[^a-z0-9]+/gi,"-")}-${exportFileDate()}.csv`,["Rang","Kundennummer","Kunde","Straße","PLZ","Ort","Ansprechpartner 1","Telefon 1","Mobil 1","E-Mail 1","Ansprechpartner 2","Telefon 2","Mobil 2","E-Mail 2","Ansprechpartner 3","Telefon 3","Mobil 3","E-Mail 3","Außendienst","Umsatz","Stand"],customers.map((c,i)=>[i+1,c.customerNumber||"",c.name||"",c.street||"",c.zip||"",c.city||"",c.contact||"",c.phone||"",c.mobile||"",c.email||"",c.contact2||"",c.phone2||"",c.mobile2||"",c.email2||"",c.contact3||"",c.phone3||"",c.mobile3||"",c.email3||"",c.owner||"",Number(c.revenue||0).toFixed(2).replace(".",","),csvExportDate(c.revenueAsOf)])); await logCsvExport("Umsatz-Rangliste",customers.length,{owner}); toast(`${customers.length} Kunden wurden als Umsatz-Rangliste exportiert.`); }
